...
public:
    ...
    // Port management
    std::shared_ptr<MachineCommPort> addSASPort();
    ...
    // Port access
    const std::vector<std::shared_ptr<MachineCommPort>>& getPorts() const { return ports_; }
    std::shared_ptr<MachineCommPort> getPrimarySASPort();
    bool hasSAS() const;
...
private:
    ...
    std::vector<std::shared_ptr<MachineCommPort>> ports_;
