...
Message MeterCommands::handleSendTotalCoinIn(simulator::Machine* machine) {
    ...
    uint64_t coinIn = machine->getMeter(SASConstants::METER_TOTAL_COIN_IN);
...
Message MeterCommands::handleSendTotalCoinOut(simulator::Machine* machine) {
    ...
    uint64_t coinOut = machine->getMeter(SASConstants::METER_TOTAL_COIN_OUT);
...
Message MeterCommands::handleSendTotalDrop(simulator::Machine* machine) {
    ...
    uint64_t drop = machine->getMeter(SASConstants::METER_TOTAL_DROP);
...
Message MeterCommands::handleSendTotalJackpot(simulator::Machine* machine) {
    ...
    uint64_t jackpot = machine->getMeter(SASConstants::METER_TOTAL_JACKPOT);
...
Message MeterCommands::handleSendGameConfiguration(simulator::Machine* machine) {
    ...
    // Get current game
    int gameNumber = machine->getSelectedGameNumber();
    response.data.push_back(BCD::toBCD(static_cast<uint8_t>(gameNumber)));

    // Add denomination code
    double denom = machine->getDenomination();
    int denomCode = SASConstants::valueToCode(denom);
    response.data.push_back(static_cast<uint8_t>(denomCode));

    // Add max bet (in credits)
    int maxBet = machine->getMaxBet();
    ...
    uint64_t gameCoinIn = machine->getMeter(SASConstants::METER_TOTAL_COIN_IN);
    ...
    uint64_t gameCoinOut = machine->getMeter(SASConstants::METER_TOTAL_COIN_OUT);
