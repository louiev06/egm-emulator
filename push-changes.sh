#!/bin/bash
# Push changes script - commits and pushes staged changes with meaningful messages

set -e

echo "========================================="
echo "Push Changes Script"
echo "========================================="
...
echo ""
echo "Step 1: Updating PLAN.md..."
cat > PLAN.md << 'EOF'
# EGM Emulator - Development Plan

## Current Status (Last Updated: $(date +"%Y-%m-%d %H:%M"))
...
sed -i "s/\$(date +\"%Y-%m-%d %H:%M\")/$(date +"%Y-%m-%d %H:%M")/g" PLAN.md
...
if echo "$STAGED_FILES" | grep -q "include.*\.h$\|src.*\.cpp$"; then
    # Code changes detected
    if git diff --cached | grep -q "recursive_mutex"; then
        COMMIT_MSG="Fix: Convert all mutexes to recursive_mutex to prevent deadlocks"
    elif git diff --cached | grep -q "METER_"; then
        COMMIT_MSG="Refactor: Rename meter constants from MTR_ to METER_ for consistency"
    elif git diff --cached | grep -q "extern \"C\""; then
        COMMIT_MSG="Fix: Add extern C linkage for s7lite library integration"
    elif git diff --cached | grep -q "BUILD_NUMBER"; then
        COMMIT_MSG="Chore: Increment build version"
    elif git diff --cached | grep -q "watchdog\|Watchdog"; then
        COMMIT_MSG="Feature: Implement watchdog timer management for Zeus OS"
    else
        COMMIT_MSG="Update: Code improvements and bug fixes"
    fi
elif echo "$STAGED_FILES" | grep -q "\.md$"; then
    COMMIT_MSG="Docs: Update project documentation"
elif echo "$STAGED_FILES" | grep -q "\.json$"; then
    COMMIT_MSG="Config: Update build configuration"
elif echo "$STAGED_FILES" | grep -q "\.sh$\|\.bat$"; then
    COMMIT_MSG="Build: Update build scripts and automation"
elif echo "$STAGED_FILES" | grep -q "\.mak$\|Makefile"; then
    COMMIT_MSG="Build: Update makefile configuration"
else
...
COMMIT_BODY=""
if [ $(echo "$STAGED_FILES" | wc -l) -le 10 ]; then
    COMMIT_BODY="
